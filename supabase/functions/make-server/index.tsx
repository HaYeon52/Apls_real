...
app.post("/make-server-40a2eee1/survey/submit", async (c) => {
  try {
    ...
    const surveyData = {
      ...
      추천받은과목수: Array.isArray(body.recommendations) ? body.recommendations.length : 0,
      추천받은과목목록: Array.isArray(body.recommendations) ? body.recommendations.map(r => `${r.semester}: ${r.name}`).join(", ") : "",
